...
class ConvertPaymentAction implements ActionInterface, GenericTokenFactoryAwareInterface
{
    ...
    public function execute($request)
    {
        ...
        $details[PayboxParams::PBX_BILLING] = $this->getBillingData($order);
        $details[PayboxParams::PBX_SHOPPINGCART] = $this->getShoppingCartData($order->getTotalQuantity());
    ...
    private function getBillingData(OrderInterface $order)
    {
        $data = $this->buildBillingArray($order);
        return $this->getXmlBilling($data);
    }

    private function getShoppingCartData(int $total)
    {
        return $this->getXmlShoppingCart($total);
    }
